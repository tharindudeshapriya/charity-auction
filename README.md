...
4. For listings, `GET /items` delegates to `ItemServiceImpl.getActiveItems(pageable)` which queries `ItemRepository.findByStatus(ItemStatus.ACTIVE, pageable)` and maps results to `ItemResponse` DTOs.
